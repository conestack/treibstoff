...
import { AjaxOperation, AjaxUtil } from '../src/ssr/util.js';
...
QUnit.module('treibstoff.ajax', (hooks) => {
    ...
    QUnit.test('Test AjaxUtil.parse_definition', (assert) => {
        ...
        assert.deepEqual(util.parse_definition('action:.selector:inner'), [
            'action:.selector:inner',
        ]);

        assert.deepEqual(util.parse_definition('a1:.sel1:inner a2:.sel2:replace'), [
            'a1:.sel1:inner',
            'a2:.sel2:replace',
        ]);

        assert.deepEqual(util.parse_definition('event:.selector'), ['event:.selector']);

        assert.deepEqual(util.parse_definition('e1:.sel1 e2:sel2'), ['e1:.sel1', 'e2:sel2']);
    });
    ...
    QUnit.test('Test AjaxPath.execute', (assert) => {
        let pushed_path, pushed_state, replaced_path, replaced_state;
        ...
        const dispatcher = new AjaxDispatcher();
        const win = new TestWindow();
        const path = new AjaxPath({
    ...
    QUnit.test('Test AjaxPath.handle_state', (assert) => {
        ...
        const trigger_popstate = (state) => {
            const evt = $.Event('popstate', { preventDefault: preventDefault });
    ...
    QUnit.test('Test AjaxAction.execute', (assert) => {
        ...
        const win = {};
        const ajax = new Ajax(win);
        const spinner = ajax.spinner;
        const request = new TestHTTPRequest({
            spinner: spinner,
            win: win,
        });
        const handle = new AjaxHandle({
            ajax: ajax,
            spinner: spinner,
        });
        const dispatcher = new AjaxDispatcher();
        const action = new AjaxAction({
            dispatcher: dispatcher,
            win: {},
            handle: handle,
    ...
    QUnit.test('Test AjaxAction.complete', (assert) => {
        ...
        let update_opts, next_operations;
        ...
        const handle = new TestAjaxHandle({
            ajax: ajax,
            spinner: spinner,
        });
        const dispatcher = new AjaxDispatcher();
        ...
        const err = $('body .modal.error').data('overlay');
    ...
    QUnit.test('Test AjaxAction.handle', (assert) => {
        ...
        const dispatcher = new AjaxDispatcher();
        const action = new TestAjaxAction({
            dispatcher: dispatcher,
            win: win,
            handle: handle,
            spinner: ajax.spinner,
    ...
    QUnit.test('Test AjaxEvent.create_event', (assert) => {
        const dispatcher = new AjaxDispatcher();
        const event = new AjaxEvent({ dispatcher: dispatcher });
        ...
        const target = event.parse_target('https://tld.com/sub?param=value');
    ...
    QUnit.test('Test AjaxEvent.execute', (assert) => {
        ...
        const elem = $(`<div class="testevent-listener"></div>`);
        $('body').append(elem);
    ...
    QUnit.test('Test AjaxEvent.handle', (assert) => {
        ...
        const dispatcher = new AjaxDispatcher();
        const _event = new TestAjaxEvent({ dispatcher: dispatcher });
    ...
    QUnit.test('Test AjaxOverlay.execute', (assert) => {
        ...
        const dispatcher = new AjaxDispatcher();
        const overlay = new TestAjaxOverlay({
            dispatcher: dispatcher,
            win: {},
            handle: handle,
            spinner: ajax.spinner,
    ...
    QUnit.test('Test AjaxOverlay.handle', (assert) => {
        ...
        const dispatcher = new AjaxDispatcher();
        const overlay = new TestAjaxOverlay({ dispatcher: dispatcher });
    ...
    QUnit.test('Test AjaxForm', (assert) => {
        ...
        const ajax = new Ajax(win);
        const spinner = ajax.spinner;
        const handle = new TestAjaxHandle(ajax);
        ...
        const hidden_iframe = $('body > #ajaxformresponse');
        ...
        assert.deepEqual(hidden_iframe.attr('style'), 'width:0px;height:0px;display:none');
        ...
        const hidden_input = $(form_elem.children()[0]);
    ...
    QUnit.test('Test AjaxDispatcher.dispatch_handle', (assert) => {
        ...
        const dispatcher = new TestAjaxDispatcher();
    ...
    QUnit.test('Test AjaxDispatcher.dispatch', (assert) => {
        ...
        let action_opts;
        dispatcher.on('on_action', (_inst, opts) => {
            action_opts = opts;
        });

        let event_opts;
        dispatcher.on('on_event', (_inst, opts) => {
            event_opts = opts;
        });

        let overlay_opts;
        dispatcher.on('on_overlay', (_inst, opts) => {
            overlay_opts = opts;
        });

        let path_opts;
        dispatcher.on('on_path', (_inst, opts) => {
            path_opts = opts;
        });
    ...
    QUnit.test('Test AjaxHandle.update', (assert) => {
        ...
        const container_inst = new Inst();
        container[0]._ajax_attached = [container_inst];

        const child = $('<div class="child" />').appendTo(container),
            child_inst = new Inst();
    ...
    QUnit.test('Test AjaxHandle.next', (assert) => {
        ...
        const ol = $('body > .modal-wrapper > .modal.info').data('overlay');
        assert.ok(ol.is_open);
        assert.deepEqual(ol.content, 'Message Content');
        ol.close();

        // case message in custom element
        const elem = $('<span class="ajax-message-content" />').appendTo($('body'));
        handle.next([
            {
                type: 'message',
                payload: 'Message Content',
    ...
    QUnit.test('Test Ajax.register', (assert) => {
        ...
        assert.verifySteps([]);
        let count = 0;
        for (const func_name in ajax.binders) {
            count++;
        ...
        count = 0;
        for (const _func_name in ajax.binders) {
            count++;
        }
    ...
    QUnit.test('Test Ajax.parseurl', (assert) => {
        const ajax = new Ajax();
        assert.deepEqual(ajax.parseurl('https://tld.com/'), 'https://tld.com');
    });

    QUnit.test('Test Ajax.parsequery', (assert) => {
        const ajax = new Ajax();
        assert.deepEqual(ajax.parsequery('https://tld.com?foo=bar'), { foo: 'bar' });
        assert.deepEqual(ajax.parsequery('https://tld.com?foo=bar', true), '?foo=bar');
    });

    QUnit.test('Test Ajax.parsepath', (assert) => {
        const ajax = new Ajax();
        assert.deepEqual(ajax.parsepath('https://tld.com/sub'), '/sub');
        assert.deepEqual(ajax.parsepath('https://tld.com/sub?foo=bar', true), '/sub?foo=bar');
    });
    ...
    QUnit.test('Test Ajax.path', (assert) => {
        ...
        const ajax = new Ajax();
        ajax._path.execute = (opts) => {
            execute_opts = opts;
        };

        const opts = { path: '/test', target: 'http://tld.com/test' };
        ajax.path(opts);
    ...
    QUnit.test('Test Ajax.overlay', (assert) => {
        ...
        const result = ajax.overlay(opts);
        assert.deepEqual(execute_opts, opts);
    ...
    QUnit.test('Test Ajax.warning', (assert) => {
        ...
        const ol = $('body > .modal-wrapper > .modal.warning').data('overlay');
        assert.ok(ol.is_open);
    ...
    QUnit.test('Test Ajax.error', (assert) => {
        ...
        const ol = $('body > .modal-wrapper > .modal.error').data('overlay');
        assert.ok(ol.is_open);
    ...
    QUnit.test('Test Ajax.dialog', (assert) => {
        ...
        const ajax = new Ajax();
        const opts = { message: 'Confirm?' };
        ajax.dialog(opts, (o) => {
            callback_opts = o;
        });

        const dialog = $('body > .modal-wrapper > .modal.dialog').data('overlay');
        assert.ok(dialog.is_open);
