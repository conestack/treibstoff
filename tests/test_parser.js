...
import { create_svg_elem, parse_svg } from '../src/utils.js';

QUnit.module('treibstoff.parser', (_hooks) => {
    ...
    QUnit.test('Test extract_number', (assert) => {
        ...
        assert.strictEqual(extract_number('1'), Number(1), 'Number extractor returns Number()');
    });
    ...
    QUnit.test('Test HTMLParser', (assert) => {
        ...
        ob.on_button_down = function (_prop) {
    ...
    QUnit.test('Test HTMLParser handle_input without t-prop', (assert) => {
        ...
        const elem = $('<div><input type="text" /></div>');
        const ob = {};
        new HTMLParser(ob).walk(elem.get(0));
    ...
    QUnit.test('Test HTMLParser handle_button without t-prop', (assert) => {
        ...
        const elem = $('<div><button>Click</button></div>');
        const ob = {};
        new HTMLParser(ob).walk(elem.get(0));
