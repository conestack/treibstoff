...
QUnit.module('treibstoff.dnd', (hooks) => {
    ...
    QUnit.module('set_scope', () => {
        QUnit.test('binds dragstart/dragend on drag element', (assert) => {
            const drag_elem = $('<div class="drag">');
            container.append(drag_elem);

            const dnd = new DnD();
            dnd.set_scope(drag_elem, null);

            assert.strictEqual(drag_elem.attr('draggable'), 'true', 'draggable attribute set');
            ...
            assert.deepEqual(events, ['dragstart', 'dragend'], 'dragend event triggered');
        ...
        QUnit.test('binds dragover/dragleave/drop on drop element', (assert) => {
            const drop_elem = $('<div class="drop">');
            ...
            assert.deepEqual(events, ['dragover', 'dragleave'], 'dragleave triggered');
            ...
            assert.deepEqual(events, ['dragover', 'dragleave', 'drop'], 'drop triggered');
        ...
        QUnit.test('only drag scope (drop = null)', (assert) => {
            ...
            assert.strictEqual(drag_elem.attr('draggable'), 'true', 'draggable set');
    ...
    QUnit.module('_drag_source', () => {
        QUnit.test('dragstart sets DnD._drag_source', (assert) => {
            ...
            assert.strictEqual(DnD._drag_source, dnd, 'set to dnd instance on dragstart');
        ...
        QUnit.test('dragend clears DnD._drag_source', (assert) => {
            ...
            drag_elem.trigger('dragend');
            assert.strictEqual(DnD._drag_source, null, 'cleared on dragend');
        ...
        QUnit.test('dragover evt.source references drag source', (assert) => {
            ...
            assert.strictEqual(received_source, dnd, 'evt.source is the drag source instance');
    ...
    QUnit.module('cross-instance', () => {
        QUnit.test('drag on instance A, drop on instance B', (assert) => {
            const drag_elem = $('<div class="item-a">');
            const drop_elem = $('<div class="item-b">');
            container.append(drag_elem, drop_elem);
            ...
            assert.strictEqual(drop_source, dnd_a, 'evt.source on B is dnd_a');
        ...
        QUnit.test('dragover on B shows source from A', (assert) => {
            ...
            assert.strictEqual(over_source, dnd_a, 'dragover evt.source on B references A');
    ...
    QUnit.module('default handler methods', () => {
        ...
        QUnit.test('drop() method called as default handler', (assert) => {
            ...
            assert.strictEqual(received_evt.source, dnd, 'evt.source set before default handler');
